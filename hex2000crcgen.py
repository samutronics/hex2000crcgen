...
class HexParser:
    ...
    def crc32_calculate(self, data):     
        ...
        data = [byte for word in data for byte in word.to_bytes(2, 'big')]
        # remember a word is 2bytes!        
        for byte in data:        
            accumulator = accumulator & 0xFFFFFFFF
            table_index = (((accumulator) >> 24) & 0xFFFFFFFF) ^ (data[parity] & 0xFFFFFFFF)
            accumulator = (((accumulator) << 8) & 0xFFFFFFFF)  ^ (table[table_index] & 0xFFFFFFFF)
            parity += 1            
    ...
    def testcrc32(self):
        ...
        data = [0xFFFF] * 0x1A
        data1 = [0x0001 ,0x0002  ,0x0003  ,0x0004  ,0x0005  ,0x0006  ,0x0007  ,0x0008  ,0x0009  ,0x0010  ,0x0011  ,0x0012  ,0x0013  ,0x0014  ,0x0015  ,0x0016  ,0x0017  ,0x0018  ,0x0019  ,0x0020  ,0x0021  ,0x0022  ,0x0023  ,0x0024  ,0x0025  ,0x0026]                
        crc32 = self.crc32_calculate(data)
    ...
    def show_summary(self):
        ...
        for i in range(0, self.blocksize):
            print(f"CRC{i}: {self.crc_buffer[i]:08X}")
